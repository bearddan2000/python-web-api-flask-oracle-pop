...
class Endpoint:
    ...
    def filter_by(self, filter, filter_val):
        ...
        sql = f'select * from pop where {filter} = :param'
        return self.client.query(sql, d)
    
    def delete(self, filter, filter_val):
        self.client.seed()
        d: dict = {'param': filter_val}
        sql = f'delete from pop where {filter} = :param'
        self.client.command(sql, d)
        return self.get_all()
    
    def insert(self, new_name, new_color):
        self.client.seed()
        d: dict = {'param1': new_name, 'param2': new_color}
        sql = "insert into pop (name, color) values (:param1, :param2)"
        self.client.command(sql, d)
        return self.get_all()
    ...
    def update(self, filter, filter_val):
        self.client.seed()
        d: dict = {'param': filter_val}
        sql = f"update pop set name = 'updated', color = 'updated' where {filter} = :param"
        self.client.command(sql, d)
        return self.get_all()